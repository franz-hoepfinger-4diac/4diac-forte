...
namespace forte::com_infra::opc_ua {
  ...
  bool CUA_ClientInformation::executeAsyncCalls() {
    return (UA_STATUSCODE_GOOD == UA_Client_run_iterate(mClient, 10));
  }
  ...
  UA_StatusCode CUA_ClientInformation::executeRead(CActionInfo &paActionInfo) {
    ...
    UA_ReadValueId *ids =
        static_cast<UA_ReadValueId *>(UA_Array_new(request.nodesToReadSize, &UA_TYPES[UA_TYPES_READVALUEID]));
    request.nodesToRead = ids;

    size_t indexOfNodePair = 0;
    for (auto itNodePair = paActionInfo.getNodePairInfo().begin(); itNodePair != paActionInfo.getNodePairInfo().end();
    ...
    UA_StatusCode retVal = UA_Client_sendAsyncReadRequest(
        mClient, &request, CUA_RemoteCallbackFunctions::readAsyncCallback, remoteCallHandle, nullptr);
  ...
  UA_StatusCode CUA_ClientInformation::executeWrite(CActionInfo &paActionInfo) {
    ...
    UA_WriteValue *ids =
        static_cast<UA_WriteValue *>(UA_Array_new(request.nodesToWriteSize, &UA_TYPES[UA_TYPES_WRITEVALUE]));
    ...
    UA_RemoteCallHandle *remoteCallHandle = new UA_RemoteCallHandle(paActionInfo, *this);
    retVal = UA_Client_sendAsyncWriteRequest(mClient, &request, CUA_RemoteCallbackFunctions::writeAsyncCallback,
                                             remoteCallHandle, nullptr);
  ...
  UA_StatusCode CUA_ClientInformation::executeCallMethod(CActionInfo &paActionInfo) {
    ...
    request.methodsToCall = static_cast<UA_CallMethodRequest *>(
        UA_Array_new(request.methodsToCallSize, &UA_TYPES[UA_TYPES_CALLMETHODREQUEST]));

    UA_CallMethodRequest *methodRequest = &request.methodsToCall[0];

    methodRequest->inputArgumentsSize = paActionInfo.getSendSize();
    methodRequest->inputArguments =
        static_cast<UA_Variant *>(UA_Array_new(methodRequest->inputArgumentsSize, &UA_TYPES[UA_TYPES_VARIANT]));
    ...
    UA_RemoteCallHandle *remoteCallHandle = new UA_RemoteCallHandle(paActionInfo, *this);
    retVal = __UA_Client_AsyncService(mClient, &request, &UA_TYPES[UA_TYPES_CALLREQUEST],
                                      CUA_RemoteCallbackFunctions::callMethodAsyncCallback,
  ...
  bool CUA_ClientInformation::connectClient() {
    if (0 == mUsername.compare("")) {
      ...
      return (UA_STATUSCODE_GOOD ==
              UA_Client_connectUsername(mClient, mEndpointUrl.c_str(), mUsername.c_str(), mPassword.c_str()));
    }
  }
  ...
  bool CUA_ClientInformation::initializeAction(CActionInfo &paActionInfo) {
    ...
    if (CActionInfo::eCallMethod == paActionInfo.getAction()) {
      ...
      for (auto itNodePair = paActionInfo.getNodePairInfo().begin(); itNodePair != paActionInfo.getNodePairInfo().end();
        ...
        if (!somethingFailed &&
            !itNodePair->getBrowsePath()
                 .empty()) { // if browsepath was given, look for NodeId, even if NodeID was also provided
          UA_NodeId *nodeId;
          UA_StatusCode retVal = COPC_UA_Helper::getRemoteNodeForPath(
              *mClient, itNodePair->getBrowsePath().c_str(), nullptr, &nodeId); // we don't care about the parent

          if (UA_STATUSCODE_GOOD != retVal) {
            DEVLOG_ERROR("[OPC UA CLIENT]: The index %u of the FB %s could not be initialized because the requested "
                         "nodeId was not found. Error: %s\n",
                         runnerHelper, paActionInfo.getLayer().getCommFB()->getInstanceName(),
                         UA_StatusCode_name(retVal));
            somethingFailed = true;
          } else {
            if (itNodePair->getNodeId() != nullptr) {
              if (!UA_NodeId_equal(itNodePair->getNodeId(),
                                   nodeId)) { // if NodeId was provided, check if found is the same
                DEVLOG_ERROR("[OPC UA CLIENT]: The call from FB %s failed the found nodeId of the method doesn't match "
                             "the provided one\n",
      ...
      if (!somethingFailed &&
          !initializeSubscription(paActionInfo)) { // won't initialize subscription if some nodeID is missing
        somethingFailed = true;
      }
    }
    return somethingFailed;
  ...
  bool CUA_ClientInformation::initializeCallMethod(CActionInfo &paActionInfo) {
    ...
    UA_StatusCode retVal =
        COPC_UA_Helper::getRemoteNodeForPath(*mClient, itNodePair->getBrowsePath().c_str(), &parentNode, &methodNode);

    if (UA_STATUSCODE_GOOD != retVal) {
      DEVLOG_ERROR(
          "[OPC UA CLIENT]: The method call from FB %s failed because the requested node was not found. Error: %s\n",
          paActionInfo.getLayer().getCommFB()->getInstanceName(), UA_StatusCode_name(retVal));
      somethingFailed = true;
    } else {
      if (itNodePair->getNodeId() != nullptr) {
        if (!UA_NodeId_equal(itNodePair->getNodeId(),
                             methodNode)) { // if NodeId of method was provided, check if found is the same
          DEVLOG_ERROR(
              "[OPC UA CLIENT]: The method call from FB %s failed the found nodeId of the method doesn't match "
              "the provided one\n",
              paActionInfo.getLayer().getCommFB()->getInstanceName());
          somethingFailed = true;
        }
  ...
  bool CUA_ClientInformation::createSubscription() {
    ...
    UA_CreateSubscriptionResponse response = UA_Client_Subscriptions_create(
        mClient, request, this, nullptr, CUA_RemoteCallbackFunctions::deleteSubscriptionCallback);
  ...
  bool CUA_ClientInformation::addMonitoringItem(UA_MonitoringItemInfo &paMonitoringInfo, const UA_NodeId &paNodeId) {
    ...
    UA_MonitoredItemCreateResult monResponse = UA_Client_MonitoredItems_createDataChange(
        mClient, mSubscriptionInfo.mSubscriptionId, UA_TIMESTAMPSTORETURN_BOTH, monRequest,
        static_cast<void *>(&paMonitoringInfo.mVariableInfo),
  ...
  void CUA_ClientInformation::CUA_RemoteCallbackFunctions::readAsyncCallback(UA_Client *,
                                                                             void *paUserdata,
                                                                             UA_UInt32,
                                                                             UA_ReadResponse *paResponse) { // NOSONAR
      ...
      if (paResponse->resultsSize == remoteCallHandle->mActionInfo.getNoOfNodePairs()) {
        ...
        for (size_t i = 0; i < paResponse->resultsSize; i++) {
          if (paResponse->results[i].hasStatus && UA_STATUSCODE_GOOD != paResponse->results[i].status) {
            DEVLOG_ERROR("[OPC UA CLIENT]: Reading for FB %s in client %s failed because the response for index %u has "
                         "status %s\n",
                         remoteCallHandle->mActionInfo.getLayer().getCommFB()->getInstanceName(),
                         remoteCallHandle->mActionInfo.getEndpoint().c_str(), i,
        ...
        DEVLOG_ERROR(
            "[OPC UA CLIENT]: Reading for FB %s in client %s failed because the response size is %u but the FB "
            "has %u values to read\n",
            remoteCallHandle->mActionInfo.getLayer().getCommFB()->getInstanceName(),
            remoteCallHandle->mActionInfo.getEndpoint().c_str(), paResponse->resultsSize,
            remoteCallHandle->mActionInfo.getNoOfNodePairs());
        varHandle.mFailed = true;
      }
    } else {
      DEVLOG_ERROR("[OPC UA CLIENT]: Reading for FB %s in client %s failed. Error: %s\n",
                   remoteCallHandle->mActionInfo.getLayer().getCommFB()->getInstanceName(),
                   remoteCallHandle->mActionInfo.getEndpoint().c_str(),
  ...
  void CUA_ClientInformation::CUA_RemoteCallbackFunctions::writeAsyncCallback(UA_Client *,
                                                                              void *paUserdata,
                                                                              UA_UInt32,
                                                                              UA_WriteResponse *paResponse) { // NOSONAR
    UA_RemoteCallHandle *remoteCallHandle = static_cast<UA_RemoteCallHandle *>(paUserdata);
    remoteCallHandle->mClientInformation.removeAsyncCall();
    COPC_UA_Helper::UA_RecvVariable_handle varHandle;
    if (UA_STATUSCODE_GOOD == paResponse->responseHeader.serviceResult) {
      if (paResponse->resultsSize == remoteCallHandle->mActionInfo.getNoOfNodePairs()) {
        for (size_t i = 0; i < paResponse->resultsSize; i++) {
          if (UA_STATUSCODE_GOOD != paResponse->results[i]) {
            DEVLOG_ERROR("[OPC UA CLIENT]: Writing for FB %s in client %s failed because the response for index %u has "
                         "status %s\n",
                         remoteCallHandle->mActionInfo.getLayer().getCommFB()->getInstanceName(),
                         remoteCallHandle->mActionInfo.getEndpoint().c_str(), i,
        ...
        DEVLOG_ERROR(
            "[OPC UA CLIENT]: Writing for FB %s in client %s failed because the response size is %u but the FB "
            "has %u values to write\n",
            remoteCallHandle->mActionInfo.getLayer().getCommFB()->getInstanceName(),
            remoteCallHandle->mActionInfo.getEndpoint().c_str(), paResponse->resultsSize,
            remoteCallHandle->mActionInfo.getNoOfNodePairs());
        varHandle.mFailed = true;
  ...
  void CUA_ClientInformation::CUA_RemoteCallbackFunctions::callMethodAsyncCallback( // We omit SONAR only for the
                                                                                    // parameters
      UA_Client *,
      void *paUserdata,
      UA_UInt32,
      void *paResponse) { // NOSONAR
    ...
    if (UA_STATUSCODE_GOOD == response->responseHeader.serviceResult) {
      if (1 == response->resultsSize) {
        if (UA_STATUSCODE_GOOD == response->results[0].statusCode) {
          ...
          if (remoteCallHandle->mActionInfo.getLayer().getCommFB()->getNumRD() !=
              response->results[0].outputArgumentsSize) {
            DEVLOG_ERROR(
                "[OPC UA CLIENT]: Calling for FB %s in client %s failed because the number of RD connectors of "
                "the client %u does not match the number of returned values %u from the method call\n",
                remoteCallHandle->mActionInfo.getLayer().getCommFB()->getInstanceName(),
                remoteCallHandle->mActionInfo.getEndpoint().c_str(),
                remoteCallHandle->mActionInfo.getLayer().getCommFB()->getNumRD(),
                response->results->outputArgumentsSize);
            somethingFailed = true;
          } else {
            for (size_t i = 0; i < response->results->inputArgumentResultsSize; i++) {
              if (UA_STATUSCODE_GOOD != response->results->inputArgumentResults[i]) {
                DEVLOG_ERROR("[OPC UA CLIENT]: Calling for FB %s in client %s failed because the input response for "
                             "index %u has status %s\n",
                             remoteCallHandle->mActionInfo.getLayer().getCommFB()->getInstanceName(),
                             remoteCallHandle->mActionInfo.getEndpoint().c_str(), i,
        ...
        DEVLOG_ERROR("[OPC UA CLIENT]: Calling for FB %s in client %s failed because the response size is %u, "
                     "different from 1\n",
                     remoteCallHandle->mActionInfo.getLayer().getCommFB()->getInstanceName(),
                     remoteCallHandle->mActionInfo.getEndpoint().c_str(), response->resultsSize);
        somethingFailed = true;
      }
    } else {
  ...
  void CUA_ClientInformation::CUA_RemoteCallbackFunctions::subscriptionValueChangedCallback(
      UA_Client *,
      UA_UInt32,
      void *,
      UA_UInt32,
      void *paMonContext, // NOSONAR
  ...
  void CUA_ClientInformation::CUA_RemoteCallbackFunctions::deleteSubscriptionCallback(
      UA_Client *, UA_UInt32 paSubscriptionId, void *paSubscriptionContext) { // NOSONAR
  ...
  void CUA_ClientInformation::CUA_RemoteCallbackFunctions::clientStateChangeCallback( // We omit SONAR only for the
                                                                                      // parameters
      UA_Client *,
    ...
    // Don't do anything here. If the subscription is deleted, deleteSubscriptionCallback will be called and handled
    // there
