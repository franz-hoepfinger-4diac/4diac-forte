 ...
 *******************************************************************************/
#include "forte/iec61131/booleanOperators/genboolbase_fbt.h"
#include <memory>
...
namespace forte::iec61131::booleanOperators {
  ...
  CGenBoolBase::CGenBoolBase(const StringId paInstanceNameId, CFBContainer &paContainer) :
      CGenFunctionBlock<CFunctionBlock>(paContainer, paInstanceNameId),
      conn_CNF(*this, 0),
      conn_OUT(*this, 0, var_OUT) {
  }

  void CGenBoolBase::readInputData(TEventID) {
    for (TPortId i = 0; i < getGenDINums(); ++i) {
      readData(i, mGenDIs[i], mGenDIConns[i]);
    }
  }

  void CGenBoolBase::writeOutputData(TEventID) {
    writeData(getFBInterfaceSpec().getNumDIs() + 0, var_OUT, conn_OUT);
  }

  bool CGenBoolBase::createInterfaceSpec(const char *paConfigString, SFBInterfaceSpec &paInterfaceSpec) {
  ...
  CIEC_ANY *CGenBoolBase::getDI(size_t paDINum) {
    return &mGenDIs[paDINum];
  }

  CIEC_ANY *CGenBoolBase::getDO(size_t paDONum) {
    return (paDONum == 0) ? &var_OUT : nullptr;
  }

  CEventConnection *CGenBoolBase::getEOConUnchecked(TPortId paEONum) {
    return (paEONum == 0) ? &conn_CNF : nullptr;
  }

  CDataConnection *CGenBoolBase::getDOConUnchecked(TPortId paDONum) {
    return (paDONum == 0) ? &conn_OUT : nullptr;
  }

  void CGenBoolBase::createGenInputData() {
    mGenDIs = std::make_unique<CIEC_BOOL[]>(getFBInterfaceSpec().getNumDIs());
  }
} // namespace forte::iec61131::booleanOperators
