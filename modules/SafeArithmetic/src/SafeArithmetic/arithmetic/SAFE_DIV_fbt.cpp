...
#include "forte/SafeArithmetic/arithmetic/SAFE_DIV_fbt.h"
...
namespace forte::SafeArithmetic::arithmetic {
  ...
  DEFINE_FIRMWARE_FB(FORTE_SAFE_DIV, "SafeArithmetic::arithmetic::SAFE_DIV"_STRID)

  FORTE_SAFE_DIV::FORTE_SAFE_DIV(const StringId paInstanceNameId, CFBContainer &paContainer) :
  ...
  void FORTE_SAFE_DIV::executeEvent(TEventID paEIID, CEventChainExecutionThread *const paECET) {
    switch (paEIID) {
      case scmEventREQID:
        ...
        var_OUT = std::visit(
            [this](auto &&paIN1, auto &&paIN2) -> CIEC_ANY_NUM_VARIANT {
              ...
              if constexpr (!std::is_same<deductedType, mpl::NullType>::value) {
                bool limitHit = false;
                auto result = safe_div(paIN1, paIN2, limitHit);
  ...
  void FORTE_SAFE_DIV::readInputData(TEventID paEIID) {
    switch (paEIID) {
      case scmEventREQID: {
  ...
  void FORTE_SAFE_DIV::writeOutputData(TEventID paEIID) {
    switch (paEIID) {
      case scmEventCNFID: {
  ...
  CIEC_ANY *FORTE_SAFE_DIV::getDI(size_t paIndex) {
    switch (paIndex) {
  ...
  CIEC_ANY *FORTE_SAFE_DIV::getDO(size_t paIndex) {
    switch (paIndex) {
  ...
  CEventConnection *FORTE_SAFE_DIV::getEOConUnchecked(TPortId paIndex) {
    switch (paIndex) {
  ...
  CDataConnection **FORTE_SAFE_DIV::getDIConUnchecked(TPortId paIndex) {
    switch (paIndex) {
  ...
  CDataConnection *FORTE_SAFE_DIV::getDOConUnchecked(TPortId paIndex) {
    switch (paIndex) {
  ...
  void FORTE_SAFE_DIV::setInitialValues() {
    var_IN1.reset();
    var_IN2.reset();
    var_OUT.reset();
    var_LIMIT_HIT = CIEC_BOOL(false);
