...
#include "forte/SafeArithmetic/arithmetic/GEN_SAFE_ADD_fbt.h"
#include "forte/SafeArithmetic/arithmetic/safe_arithmetic_ops.h"

using namespace forte::literals;

namespace forte::SafeArithmetic::arithmetic {
  DEFINE_GENERIC_FIRMWARE_FB(GEN_SAFE_ADD, "SafeArithmetic::arithmetic::GEN_SAFE_ADD"_STRID)

  GEN_SAFE_ADD::GEN_SAFE_ADD(const StringId paInstanceNameId, CFBContainer &paContainer) :
      CGenSafeArithBase<CIEC_ANY_MAGNITUDE_VARIANT>(paInstanceNameId, paContainer) {
  }

  void GEN_SAFE_ADD::executeEvent(TEventID paEIID, CEventChainExecutionThread *const paECET) {
    if (paEIID == scmEventREQID) {
      var_OUT = var_IN(0);
      bool limitHit = false;
      for (size_t i = 1; i < getFBInterfaceSpec().getNumDIs(); ++i) {
        var_OUT = std::visit(
            [&limitHit](auto &&paOUT, auto &&paIN) -> CIEC_ANY_MAGNITUDE_VARIANT {
              ...
              if constexpr (!std::is_same<deductedType, mpl::NullType>::value) {
                ...
                auto result = safe_add(paOUT, paIN, stepLimitHit);
