...
namespace forte::eclipse4diac::convert::test {
  ...
  BOOST_AUTO_TEST_CASE(missingDataType) {
    CFunctionBlock *fb = createFB("InstanceName"_STRID, "eclipse4diac::convert::STRUCT_MUX_1UnknownType"_STRID,
                                  forte::test::CFBContainerMock::smDefaultFBContMock);
    BOOST_CHECK(nullptr == fb);
  }

  BOOST_AUTO_TEST_CASE(wrongDataType) {
    CFunctionBlock *fb = createFB("InstanceName"_STRID, "eclipse4diac::convert::STRUCT_MUX_1STRING"_STRID,
                                  forte::test::CFBContainerMock::smDefaultFBContMock);
    BOOST_CHECK(nullptr == fb);

    fb = createFB("InstanceName"_STRID, "eclipse4diac::convert::STRUCT_MUX_1REAL"_STRID,
                  forte::test::CFBContainerMock::smDefaultFBContMock);
    BOOST_CHECK(nullptr == fb);
  }
  ...
  BOOST_AUTO_TEST_CASE(emptyStruct) {
    CFunctionBlock *fb =
        createFB("InstanceName"_STRID, "eclipse4diac::convert::STRUCT_MUX_1Struct_Muxer_Test_Struct_3"_STRID,
                 forte::test::CFBContainerMock::smDefaultFBContMock);
    BOOST_CHECK(nullptr == fb);
  }
        ...
        for (int j = 0; j <= 1; ++j) {
          for (int i = 0; i <= 3; ++i) {
            BOOST_TEST(static_cast<CIEC_INT::TValueType>(mVar3[j][i]) ==
                       static_cast<CIEC_INT::TValueType>(mOut.Var3[j][i]));
          }
        }
      }
  };
