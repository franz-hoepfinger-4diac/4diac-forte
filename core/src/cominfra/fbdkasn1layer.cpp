...
namespace forte::com_infra {
  ...
  EComResponse CFBDKASN1ComLayer::sendData(void *paData, unsigned int paSize) {
    ...
    if (mBottomLayer != nullptr) {
      ...
      int ser_size =
          -1; // stays negative if no Buffer for serialization is provided or error occurs while serialization
  ...
  EComResponse CFBDKASN1ComLayer::recvData(const void *paData, unsigned int paSize) {
    ...
    if (mFb != nullptr) {
      ...
      while (usedBufferSize != 0) {
        ...
        // required if e.g. the first element is an bool and the second element is "fragmented" - otherwise the resize
        // in the buffer will not be resized
        receivedData += nBuf;

        if (mDOPos >= unNumRD) {
          mDOPos = 0;
          eRetVal = e_ProcessDataOk;
          // FIXME if at this point data is left it is from a second run. Better first deliver this packet and
          // afterwards handle second run
        }
      }
    }
    return eRetVal;
  ...
  int CFBDKASN1ComLayer::deserializeValueString(const TForteByte *paBytes, int paStreamSize, CIEC_STRING &paIECData) {
    int nRetVal = -1;
    if (paStreamSize >= 2) {
      TForteUInt16 unStringSize = static_cast<TForteUInt16>(((((TForteUInt16) paBytes[0]) << 8) & 0xFF00) +
                                                            ((TForteUInt16) paBytes[1] & 0x00FF));
  ...
  int CFBDKASN1ComLayer::deserializeArray(const TForteByte *paBytes, int paStreamSize, CIEC_ARRAY &paArray) {
    ...
    if (paStreamSize >= 2) {
      TForteUInt16 nSize = static_cast<TForteUInt16>(((((TForteUInt16) paBytes[0]) << 8) & 0xFF00) +
                                                     ((TForteUInt16) paBytes[1] & 0x00FF));
