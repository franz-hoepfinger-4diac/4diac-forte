...
#define S2WS(x) forte_stringToWstring(x)
  ...
  CWin32SocketInterface::TSocketDescriptor CWin32SocketInterface::openTCPServerConnection(char *paIPAddr,
  ...
  CWin32SocketInterface::TSocketDescriptor CWin32SocketInterface::openTCPClientConnection(char *paIPAddr,
                                                                                          unsigned short paPort) {
    TSocketDescriptor nRetVal = INVALID_SOCKET;
    TSocketDescriptor nSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    ...
    if (INVALID_SOCKET != nSocket) {
      ...
      if (WSAStringToAddressW((LPWSTR) S2WS(paIPAddr).c_str(), AF_INET, nullptr, (LPSOCKADDR) &stSockAddr,
                              &stSockAddrSz)) {
  ...
  CWin32SocketInterface::TSocketDescriptor
  CWin32SocketInterface::acceptTCPConnection(TSocketDescriptor paListeningSockD) {
  ...
  CWin32SocketInterface::TSocketDescriptor CWin32SocketInterface::openUDPSendPort(char *paIPAddr,
                                                                                  unsigned short paPort,
    ...
    if (INVALID_SOCKET != nRetVal) {
      *mDestAddr = TUDPDestAddr();
      ...
      if (WSAStringToAddressW((LPWSTR) S2WS(paIPAddr).c_str(), AF_INET, nullptr, (LPSOCKADDR) mDestAddr,
                              &mDestAddrSz)) {
        DEVLOG_ERROR("CWin32SocketInterface: WSAStringToAddressW() failed: %d - %s\n", mDestAddr->sin_addr.s_addr,
                     paIPAddr);
      }
    ...
    if (paMCInterface) {
      ...
      if (WSAStringToAddressW((LPWSTR) S2WS(const_cast<char *>(paMCInterface)).c_str(), AF_INET, NULL,
                              (LPSOCKADDR) &stMCastIF, &stMCastIFSz)) {
        DEVLOG_ERROR("CWin32SocketInterface: WSAStringToAddressW() failed: %d - %s\n", stMCastIF.sin_addr.s_addr,
                     paMCInterface);
      }
  ...
  CWin32SocketInterface::TSocketDescriptor
  CWin32SocketInterface::openUDPReceivePort(char *paIPAddr, unsigned short paPort, const char *paMCInterface) {
    ...
    if (INVALID_SOCKET != nSocket) {
      ...
      if (int nOptVal = 1; 0 <= setsockopt(nSocket, SOL_SOCKET, SO_REUSEADDR, (char *) &nOptVal, sizeof(nOptVal))) {
        ...
        if (0 == bind(nSocket, (struct sockaddr *) &stSockAddr, sizeof(struct sockaddr))) {
          ...
          if (WSAStringToAddressW((LPWSTR) S2WS(paIPAddr).c_str(), AF_INET, nullptr, (LPSOCKADDR) &stMCastAddr,
                                  &stMCastAddrSz)) {
            DEVLOG_ERROR("CWin32SocketInterface: WSAStringToAddressW() failed: %d - %s\n", stMCastAddr.sin_addr.s_addr,
                         paIPAddr);
          }
          ...
          // if this fails we may have given a non multicasting addr. For now we accept this. May need to be changed in
          // the future.
  ...
  int CWin32SocketInterface::sendDataOnUDP(TSocketDescriptor paSockD,
